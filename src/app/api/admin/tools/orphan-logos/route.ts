...
export async function DELETE(req: Request) {
  ...
  // Strictly filter paths: must start with public/logo/, contain no directory traversal ('..'), and have image extension
  const paths = (body.paths ?? []).filter(
    (p) =>
      typeof p === "string" &&
      p.startsWith("public/logo/") &&
      !p.includes("..") &&
      !/[<>:"|?*]/.test(p) &&
      /\.(png|jpe?g|webp|svg|gif|ico|avif)$/i.test(p),
  );
