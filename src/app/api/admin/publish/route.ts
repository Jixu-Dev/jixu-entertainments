...
function sanitizeSite(s: unknown): Site | null {
  ...
  const urlTrimmed = o.url.trim();
  // Enforce valid http or https URL protocol scheme
  if (!/^https?:\/\//i.test(urlTrimmed)) return null;

  const site: Site = { name: o.name.trim(), url: urlTrimmed, logo: o.logo };
...
export async function POST(req: Request) {
  ...
  const rawRegion = (body.region ?? "").toUpperCase().trim();
  const region = rawRegion.replace(/[^A-Z0-9_-]/g, "");
