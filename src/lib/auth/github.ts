...
export function isAllowedAdmin(username: string, permission: RepoPermission): boolean {
  const repoOwner = env.REPO_OWNER().toLowerCase();
  // Owner is always authorized
  if (username.toLowerCase() === repoOwner) return true;
  // Collaborator must have explicit admin/maintain/write access
