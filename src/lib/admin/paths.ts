export function linksPathForRegion(regionCode: string): string {
  const clean = (regionCode || "").toUpperCase().replace(/[^A-Z0-9_-]/g, "");
  if (!clean || clean === "USA") return "public/links.json";
  return `public/Region-Links/links.${clean}.json`;
}
...
export function logoPathForCategory(categoryId: string, fileName: string): string {
  const safeCat = (categoryId || "general").toLowerCase().replace(/[^a-z0-9_-]/g, "_");
  const safeFile = (fileName || "logo.png")
    .replace(/[^a-zA-Z0-9._-]/g, "_")
    .replace(/\.+/g, ".");
  return `public/logo/${safeCat}/${safeFile}`;
}
